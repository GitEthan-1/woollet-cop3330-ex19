...
public class App {
	public static void main(String[] args) {
		...
		int weight = -1;
		while (weight < 0){
			weight =isPosInt(in.nextLine());
		}
		
		double bmi = (1.0 * weight / (height * height)) * 703;
		System.out.println("Your BMI " + Math.round(bmi *10)/10);
		
		System.out.println("You are " +
				(bmi < 25 && bmi > 18.5 ? "within the ideal weight range." :
						bmi < 18.5 ? "You are underweight. You should see your doctor." :
								"You are overweight. You should see your doctor.") );
	}
	
	
	// checks to see if the user inputs a pos integer value
